...
class AppointmentSummaryAdapter(private val context: Context, private val infoList: List<Service>) :
    RecyclerView.Adapter<AppointmentSummaryAdapter.AppointmentSummaryHolder>() {
    lateinit var viewModel: AppointmentViewModel
    lateinit var binding: ItemBookServiceBinding


    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): AppointmentSummaryHolder {
        binding = ItemBookServiceBinding.inflate(LayoutInflater.from(parent.context), parent, false)
        viewModel = ViewModelProvider(context as AppointmentSummaryActivity)[AppointmentViewModel::class.java]
        return AppointmentSummaryHolder(binding)
    }

    override fun onBindViewHolder(holder: AppointmentSummaryHolder, position: Int) {
        holder.apply {
    ...
    inner class AppointmentSummaryHolder(val binding: ItemBookServiceBinding) :
